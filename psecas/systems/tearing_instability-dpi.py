...
class TearingGyrotropicMHD:
	...
	def __init__(self, grid, kx=0, ky=0, z1=-0.5, z2=0.5, a=1, \
				    S=1e4, Pr=0, β=0, Δβ=0, ɣpar=3, ɣper=2, ϵ=0, periodic=True):
		import numpy as np
		...
		self.ϵ    = ϵ
		self.λ    = kx * a * np.sqrt((2 - Δβ) / (2 + (ɣpar + ɣper - 2) * β + ɣpar * Δβ))
		...
		if np.isclose(ky, 0.0):
			...
			if not self.isothermal:
				wy_rhs_ani += " -kx**2*Bx*sqrtG0*(Bx*dz(dpi) +2*By**2/δ*dpi)"
				wz_rhs_ani += " -1j*kx*Bx*By*sqrtG0*dpi"
			...
			if not self.isothermal:
				dp_rhs_adv = ""
				dp_rhs_str = " +sqrtG0*kx*Bx*(Bx*dz(duz) -1j*kx*By*duy)"
				dp_rhs_res = " +2*η*Γ2*By/δ*(1j*By*(dz(dbz,2) -kx**2*dbz) -kx*Bx*dz(dby))/sqrtG0"
				dp_rhs_vis = ""
			...
			if self.isothermal:
				...
				self.variables = ["duz", "dbz", "duy", "dby", "dpi"]
				self.labels = [
					r"$\delta u_z$",
					r"$\delta B_z$",
					r"$\delta u_y$",
